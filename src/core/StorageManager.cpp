...
void StorageManager::createSampleData() {
    ...
    // 示例学生1
    Card card1(QStringLiteral("C001"), QStringLiteral("张三"), 
               QStringLiteral("B17010101"), 100.0);
    card1.setPassword(QStringLiteral("123456"));
    sampleCards.append(card1);
    
    // 示例学生2
    Card card2(QStringLiteral("C002"), QStringLiteral("李四"), 
               QStringLiteral("B17010102"), 50.0);
    card2.setPassword(QStringLiteral("123456"));
    sampleCards.append(card2);
    
    // 示例学生3
    Card card3(QStringLiteral("C003"), QStringLiteral("王五"), 
               QStringLiteral("B17010103"), 200.0);
    card3.setPassword(QStringLiteral("123456"));
    ...
    // 创建示例上机记录
    Record record1 = Record::createNew(QStringLiteral("C001"), QStringLiteral("机房A101"));
    record1.endSession();
...
void StorageManager::generateMockData(int cardCount, int recordsPerCard) {
    ...
    QStringList surnames = {
        QStringLiteral("张"), QStringLiteral("李"), QStringLiteral("王"), QStringLiteral("刘"),
        QStringLiteral("陈"), QStringLiteral("杨"), QStringLiteral("赵"), QStringLiteral("黄"),
        QStringLiteral("周"), QStringLiteral("吴"), QStringLiteral("徐"), QStringLiteral("孙")
    };
    QStringList names = {
    ...
    QStringList locations = {
        QStringLiteral("机房A101"), QStringLiteral("机房A102"), QStringLiteral("机房B201"),
        QStringLiteral("机房B202"), QStringLiteral("图书馆电子阅览室"), QStringLiteral("实验楼C301")
    };
    ...
    for (int i = 0; i < cardCount; ++i) {
        ...
        card.setPassword(QStringLiteral("123456"));
        ...
        for (int j = 0; j < recordsPerCard; ++j) {
            ...
            // 创建记录
            Record record;
            record = Record::createNew(cardId, location);
            // 手动设置时间和结束状态
