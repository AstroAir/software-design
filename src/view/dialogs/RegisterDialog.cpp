...
RegisterDialog::RegisterDialog(CardController* cardController, QWidget* parent)
    : ElaDialog(parent), m_cardController(cardController) {
    initUI();
    initConnections();
}
...
void RegisterDialog::initConnections() {
    // 生成卡号按钮
    connect(m_generateIdBtn, &ElaPushButton::clicked, this, &RegisterDialog::onGenerateCardId);

    // 注册按钮
    connect(m_registerBtn, &ElaPushButton::clicked, this, &RegisterDialog::onRegisterClicked);

    // 回车键注册
    connect(m_confirmPwdEdit, &ElaLineEdit::returnPressed, this, &RegisterDialog::onRegisterClicked);

    // 连接Controller信号
    connect(m_cardController, &CardController::cardCreated, this, &RegisterDialog::onCardCreated);
    connect(m_cardController, &CardController::cardCreateFailed, this,
            &RegisterDialog::onCardCreateFailed);
}
...
QString RegisterDialog::generateCardId() {
    ...
    // 确保卡号不重复
    while (m_cardController->cardExists(cardId)) {
...
void RegisterDialog::onGenerateCardId() {
    m_cardIdEdit->setText(generateCardId());
}
...
bool RegisterDialog::validateInputs() {
    ...
    // 卡号格式验证
    QRegularExpression cardIdRegex(QStringLiteral("^[A-Za-z0-9]{3,20}$"));
    if (!cardIdRegex.match(cardId).hasMatch()) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight, QStringLiteral("提示"),
                               QStringLiteral("卡号格式错误：只允许字母和数字，长度3-20位"), 3000,
                               this);
        return false;
    }
    ...
    // 检查卡号是否已存在
    if (m_cardController->cardExists(cardId)) {
...
void RegisterDialog::onRegisterClicked() {
    ...
    // 发起注册请求
    m_cardController->handleRegisterCard(cardId, name, studentId, password);
}

void RegisterDialog::onCardCreated(const QString& cardId) {
...
void RegisterDialog::onCardCreateFailed(const QString& message) {
    ElaMessageBar::error(ElaMessageBarType::TopRight, QStringLiteral("错误"), message, 2000, this);
}

void RegisterDialog::resetInputs() {
    m_nameEdit->clear();
