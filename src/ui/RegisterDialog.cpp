...
void RegisterDialog::initConnections() {
    // 生成卡号按钮
    connect(m_generateIdBtn, &ElaPushButton::clicked, this, [this]() {
        m_cardIdEdit->setText(generateCardId());
    });
    
    // 注册按钮
    connect(m_registerBtn, &ElaPushButton::clicked,
            this, &RegisterDialog::onRegisterClicked);
    
    // 回车键注册
    connect(m_confirmPwdEdit, &ElaLineEdit::returnPressed,
            this, &RegisterDialog::onRegisterClicked);
}
...
void RegisterDialog::onRegisterClicked() {
    ...
    if (!cardIdRegex.match(cardId).hasMatch()) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight,
            QStringLiteral("提示"), QStringLiteral("卡号格式错误：只允许字母和数字，长度6-20位"), 3000, this);
        return;
    }
    ...
    if (name.isEmpty()) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight,
            QStringLiteral("提示"), QStringLiteral("请输入姓名"), 2000, this);
        return;
    }
    ...
    if (name.length() < 2 || name.length() > 20) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight,
            QStringLiteral("提示"), QStringLiteral("姓名长度应为2-20个字符"), 2000, this);
        return;
    }
    ...
    if (studentId.isEmpty()) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight,
            QStringLiteral("提示"), QStringLiteral("请输入学号"), 2000, this);
        return;
    }
    ...
    if (!studentIdRegex.match(studentId).hasMatch()) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight,
            QStringLiteral("提示"), QStringLiteral("学号格式错误：只允许字母和数字，长度6-20位"), 3000, this);
        return;
    }
    ...
    if (password.length() < 4 || password.length() > 20) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight,
            QStringLiteral("提示"), QStringLiteral("密码长度应为4-20位"), 2000, this);
        return;
    }
    ...
    if (m_cardManager->createCard(cardId, name, studentId, 0.0)) {
        ...
        ElaMessageBar::error(ElaMessageBarType::TopRight,
            QStringLiteral("错误"), QStringLiteral("注册失败，请重试"), 2000, this);
    }
}
