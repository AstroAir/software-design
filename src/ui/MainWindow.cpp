...
void MainWindow::initUI() {
    ...
    // 连接导航信号 - 处理页脚节点点击
    connect(this, &ElaWindow::navigationNodeClicked, this, [this](ElaNavigationType::NavigationNodeType nodeType, const QString& nodeKey) {
        Q_UNUSED(nodeType)
...
void MainWindow::initConnections() {
    ...
    // 管理员界面登出请求
    connect(m_adminDashboard, &AdminDashboard::logoutRequested, 
            m_authManager, &AuthManager::logout);
    
    // 学生界面登出请求
    connect(m_studentDashboard, &StudentDashboard::logoutRequested, 
            m_authManager, &AuthManager::logout);
...
void MainWindow::showWelcomePage() {
    if (!m_welcomePage) {
        ...
        // 功能卡片样式
        auto createFeatureCard = [this](const QString& icon, const QString& title, const QString& desc) -> QFrame* {
        ...
        // 学生功能卡片
        cardsLayout->addWidget(createFeatureCard(
            QStringLiteral("💳"),
            QStringLiteral("余额查询"),
        ...
        guideFrame->setStyleSheet(QStringLiteral(
            "QFrame#GuideFrame {"
...
void MainWindow::onLoginSucceeded(UserRole role, const QString& cardId) {
    if (role == UserRole::Admin) {
        ...
        ElaMessageBar::success(ElaMessageBarType::TopRight, 
            QStringLiteral("登录成功"), QStringLiteral("欢迎管理员"), 2000, this);
    } else {
        // 获取学生信息
        Card* card = m_cardManager->findCard(cardId);
        if (card) {
            setUserInfoCardSubTitle(card->name() + QStringLiteral(" (") + cardId + QStringLiteral(")"));
...
void MainWindow::onLogout() {
    ...
    ElaMessageBar::information(ElaMessageBarType::TopRight, 
        QStringLiteral("已登出"), QStringLiteral("您已安全退出系统"), 2000, this);
}
