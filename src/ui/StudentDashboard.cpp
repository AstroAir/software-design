...
StudentDashboard::StudentDashboard(CardManager* cardManager,
...
void StudentDashboard::initUI() {
    ...
    m_startDateEdit->setDate(QDate::currentDate().addDays(-30));  // 默认显示最近30天
...
void StudentDashboard::initConnections() {
    // 上机操作按钮
    connect(m_startSessionBtn, &ElaPushButton::clicked, 
            this, &StudentDashboard::onStartSessionClicked);
    connect(m_endSessionBtn, &ElaPushButton::clicked, 
            this, &StudentDashboard::onEndSessionClicked);
    
    // 修改密码按钮
    connect(m_changePasswordBtn, &ElaPushButton::clicked,
            this, &StudentDashboard::onChangePasswordClicked);
    
    // 登出按钮
    connect(m_logoutBtn, &ElaPushButton::clicked, 
            this, &StudentDashboard::logoutRequested);
    
    // 日期筛选变化
    connect(m_startDateEdit, &QDateEdit::dateChanged,
            this, &StudentDashboard::onDateFilterChanged);
    connect(m_endDateEdit, &QDateEdit::dateChanged,
            this, &StudentDashboard::onDateFilterChanged);
    connect(m_locationFilter, QOverload<int>::of(&ElaComboBox::currentIndexChanged),
            this, &StudentDashboard::onDateFilterChanged);
...
void StudentDashboard::setCurrentCard(const QString& cardId) {
...
void StudentDashboard::updateLocationFilter() {
    // 获取所有记录中的地点
    QList<Record> records = m_recordManager->getRecords(m_currentCardId);
...
void StudentDashboard::refresh() {
    if (m_currentCardId.isEmpty()) return;
...
void StudentDashboard::updateCardInfo() {
    Card* card = m_cardManager->findCard(m_currentCardId);
    if (!card) return;
    ...
    if (card->state() == CardState::Normal) {
        m_statusLabel->setStyleSheet(QStringLiteral("color: #27AE60;"));  // 绿色
    } else if (card->state() == CardState::Lost) {
        m_statusLabel->setStyleSheet(QStringLiteral("color: #F39C12;"));  // 橙色
    } else {
        m_statusLabel->setStyleSheet(QStringLiteral("color: #E74C3C;"));  // 红色
    }
}

void StudentDashboard::updateSessionStatus() {
    // 检查卡状态
    Card* card = m_cardManager->findCard(m_currentCardId);
    ...
    bool isOnline = m_recordManager->isOnline(m_currentCardId);
    
    if (isOnline) {
        Record* session = m_recordManager->getCurrentSession(m_currentCardId);
        if (session) {
            ...
            // 显示当前费用
            double currentCost = session->calculateCurrentCost();
            m_sessionTimeLabel->setText(m_sessionTimeLabel->text() + 
                QStringLiteral("  当前费用：") + QString::number(currentCost, 'f', 2) + QStringLiteral(" 元"));
        }
        m_startSessionBtn->setEnabled(false);
...
void StudentDashboard::onDateFilterChanged() {
    if (m_currentCardId.isEmpty()) return;
    
    // 获取筛选条件
    QDate startDate = m_startDateEdit->date();
    QDate endDate = m_endDateEdit->date();
    QString locationFilter = m_locationFilter->currentText();
    
    // 获取所有记录
    QList<Record> allRecords = m_recordManager->getRecords(m_currentCardId);
    ...
    for (const auto& record : allRecords) {
        ...
        // 地点筛选
        if (locationFilter != QStringLiteral("全部地点") && record.location() != locationFilter) {
            continue;
        }
    ...
    // 更新表格
    m_recordTable->setRecords(filteredRecords);
}
...
void StudentDashboard::onChangePasswordClicked() {
    ...
    ElaMessageBar::success(ElaMessageBarType::TopRight,
        QStringLiteral("成功"), QStringLiteral("密码修改成功"), 2000, this);
}

void StudentDashboard::onStartSessionClicked() {
    if (m_currentCardId.isEmpty()) return;
    ...
    if (!card || !card->isUsable()) {
        ElaMessageBar::error(ElaMessageBarType::TopRight,
            QStringLiteral("错误"), QStringLiteral("卡片状态异常，无法上机"), 2000, this);
        return;
    }
    ...
    if (card->balance() <= 0) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight,
            QStringLiteral("提示"), QStringLiteral("余额不足，请先充值"), 2000, this);
        return;
    }
    
    // 选择上机地点
    bool ok;
    QString location = QInputDialog::getText(this,
        QStringLiteral("开始上机"),
        QStringLiteral("请输入上机地点："),
        QLineEdit::Normal,
        QStringLiteral("机房A101"), &ok);
    
    if (ok && !location.isEmpty()) {
        Record record = m_recordManager->startSession(m_currentCardId, location);
        if (!record.recordId().isEmpty()) {
            ElaMessageBar::success(ElaMessageBarType::TopRight,
                QStringLiteral("成功"), QStringLiteral("上机开始"), 2000, this);
            refresh();
        } else {
            ElaMessageBar::error(ElaMessageBarType::TopRight,
                QStringLiteral("错误"), QStringLiteral("开始上机失败"), 2000, this);
        }
    }
}

void StudentDashboard::onEndSessionClicked() {
    if (m_currentCardId.isEmpty()) return;
    
    double cost = m_recordManager->endSession(m_currentCardId);
    if (cost >= 0) {
        // 扣款
        m_cardManager->deductFromCard(m_currentCardId, cost);
        
        ElaMessageBar::success(ElaMessageBarType::TopRight,
            QStringLiteral("成功"), 
            QStringLiteral("上机结束，本次费用：") + QString::number(cost, 'f', 2) + QStringLiteral(" 元"),
            3000, this);
        refresh();
    } else {
        ElaMessageBar::error(ElaMessageBarType::TopRight,
            QStringLiteral("错误"), QStringLiteral("结束上机失败"), 2000, this);
    }
}
