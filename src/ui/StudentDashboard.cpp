...
StudentDashboard::StudentDashboard(CardManager* cardManager, RecordManager* recordManager,
                                   QWidget* parent)
...
void StudentDashboard::initConnections() {
    // 上机操作按钮
    connect(m_startSessionBtn, &ElaPushButton::clicked, this,
            &StudentDashboard::onStartSessionClicked);
    connect(m_endSessionBtn, &ElaPushButton::clicked, this, &StudentDashboard::onEndSessionClicked);
    ...
    // 登出按钮
    connect(m_logoutBtn, &ElaPushButton::clicked, this, &StudentDashboard::logoutRequested);

    // 日期筛选变化
    connect(m_startDateEdit, &QDateEdit::dateChanged, this, &StudentDashboard::onDateFilterChanged);
    connect(m_endDateEdit, &QDateEdit::dateChanged, this, &StudentDashboard::onDateFilterChanged);
    connect(m_locationFilter, QOverload<int>::of(&ElaComboBox::currentIndexChanged), this,
            &StudentDashboard::onDateFilterChanged);

    // 记录变更信号
    connect(m_recordManager, &RecordManager::recordsChanged, this, [this](const QString& cardId) {
...
void StudentDashboard::updateSessionStatus() {
    ...
    if (isOnline) {
        ...
        if (session) {
            ...
            QString startTime =
                session->startTime().toString(QStringLiteral("yyyy-MM-dd HH:mm:ss"));
            ...
            m_sessionTimeLabel->setText(
                m_sessionTimeLabel->text() + QStringLiteral("  当前费用：") +
...
void StudentDashboard::onStartSessionClicked() {
    ...
    if (!card || !card->isUsable()) {
        ElaMessageBar::error(ElaMessageBarType::TopRight, QStringLiteral("错误"),
                             QStringLiteral("卡片状态异常，无法上机"), 2000, this);
        return;
    }
    ...
    if (card->balance() <= 0) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight, QStringLiteral("提示"),
                               QStringLiteral("余额不足，请先充值"), 2000, this);
        return;
    }
    ...
    if (ok && !location.isEmpty()) {
        ...
        if (!record.recordId().isEmpty()) {
            ...
            ElaMessageBar::error(ElaMessageBarType::TopRight, QStringLiteral("错误"),
                                 QStringLiteral("开始上机失败"), 2000, this);
        }
    }
}
...
void StudentDashboard::onEndSessionClicked() {
    ...
    if (cost >= 0) {
        ...
        ElaMessageBar::error(ElaMessageBarType::TopRight, QStringLiteral("错误"),
                             QStringLiteral("结束上机失败"), 2000, this);
    }
}
