...
RechargeDialog::RechargeDialog(CardManager* cardManager,
...
void RechargeDialog::initUI() {
    ...
    if (card) {
        // 卡信息
        m_cardInfoLabel = new ElaText(
            QStringLiteral("卡号：") + m_cardId + 
            QStringLiteral("  姓名：") + card->name(), centralWidget);
        layout->addWidget(m_cardInfoLabel);
        
        // 当前余额
        m_balanceLabel = new ElaText(
            QStringLiteral("当前余额：") + 
            QString::number(card->balance(), 'f', 2) + 
            QStringLiteral(" 元"), centralWidget);
...
void RechargeDialog::onRechargeClicked() {
    ...
    if (amountStr.isEmpty()) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight,
            QStringLiteral("提示"), QStringLiteral("请输入充值金额"), 2000, this);
        return;
    }
    ...
    if (!ok || amount <= 0) {
        ElaMessageBar::warning(ElaMessageBarType::TopRight,
            QStringLiteral("提示"), QStringLiteral("请输入有效的金额"), 2000, this);
        return;
    }
    ...
    if (m_cardManager->rechargeCard(m_cardId, amount)) {
        ...
        ElaMessageBar::error(ElaMessageBarType::TopRight,
            QStringLiteral("错误"), QStringLiteral("充值失败"), 2000, this);
    }
}
