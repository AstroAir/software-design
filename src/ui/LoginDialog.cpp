...
LoginDialog::LoginDialog(AuthManager* authManager, QWidget* parent)
    : ElaDialog(parent), m_authManager(authManager), m_cardManager(nullptr) {
    initUI();
    initConnections();
}
...
void LoginDialog::onLoginClicked() {
        ...
        if (m_cardManager) {
            ...
            if (card) {
                if (card->state() == CardState::Frozen) {
                    ...
                    ElaMessageBar::error(
                        ElaMessageBarType::TopRight, QStringLiteral("登录失败"),
    ...
    if (result == AuthManager::LoginResult::Success) {
        ...
        ElaMessageBar::error(ElaMessageBarType::TopRight, QStringLiteral("登录失败"), errorMsg,
                             3000, this);
    }
}
...
void LoginDialog::onRegisterClicked() {
    if (!m_cardManager) {
        ElaMessageBar::error(ElaMessageBarType::TopRight, QStringLiteral("错误"),
                             QStringLiteral("无法打开注册界面"), 2000, this);
        return;
    }
